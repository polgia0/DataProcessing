...
Ui_univariaterowDialog,QDialog=loadUiType('univariaterow.ui')
class univariaterowDlg(QtWidgets.QDialog,Ui_univariaterowDialog):
    ...
    def redraw(self):
        ...
        if self.XcomboBox.currentText()=='all' and not self.trendradioButton.isChecked():
            QtWidgets.QMessageBox.critical(self,'Error',"All rows can be plotted \n only using : Trend !",QtWidgets.QMessageBox.Ok)
            return()
        color='blue'
        ...
        if(self.BoxradioButton.isChecked()):
            ...
            if Nnan:
                ax.annotate('NaN present',xy=(0.80,0.95),xycoords='axes fraction')
            ...
            ax1=fig.add_subplot(2,2,1,title="Original "+self.XcomboBox.currentText())
            ax1.hist(Y,bins=bins,histtype='bar',color=color, alpha=0.5, orientation="vertical",label="x")
            if Nnan:
                ax1.annotate('NaN present',xy=(0.80,0.95),xycoords='figure fraction')
            ...
            ax3=fig.add_subplot(2,2,3,title="Original "+self.XcomboBox.currentText())
            ...
            fig=Figure()
            ax1=fig.add_subplot(1,2,1,title="Historgram of "+self.XcomboBox.currentText())
